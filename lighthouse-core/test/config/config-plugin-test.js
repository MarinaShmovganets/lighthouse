...
const nicePluginPath = 'nice-plugin/plugin.js'
const nicePlugin = {
...
describe('ConfigPlugin', () => {
  it('accepts a well formed plugin', () => {
    const pluginJson = ConfigPlugin.parsePlugin(nicePlugin, nicePluginName, nicePluginPath);
    expect(pluginJson).toMatchSnapshot();
  });

  it('throws on something other than a plugin object', () => {
    assert.throws(() => ConfigPlugin.parsePlugin(5, nicePluginName, nicePluginPath),
      /^Error: lighthouse-plugin-nice-plugin is not defined as an object/);
  });

  it('throws on an array instead of a plugin object', () => {
    assert.throws(() => ConfigPlugin.parsePlugin([], nicePluginName, nicePluginPath),
      /^Error: lighthouse-plugin-nice-plugin is not defined as an object/);
  });
  ...
  it('throws if there are excess plugin properties', () => {
    ...
    assert.throws(() => ConfigPlugin.parsePlugin(pluginClone, nicePluginName, nicePluginPath),
  ...
  describe('`audits` array', () => {
    it('correctly passes through the contained audits', () => {
      ...
      const pluginJson = ConfigPlugin.parsePlugin(pluginClone, nicePluginName, nicePluginPath);
  ...
  describe('`category`', () => {
    ...
    it('throws if category has an invalid or missing title', () => {
      ...
      assert.throws(() => ConfigPlugin.parsePlugin(pluginClone2, nicePluginName, nicePluginPath),
        /^Error: lighthouse-plugin-nice-plugin has an invalid category tile/);
    });
  ...
  describe('`locales`', () => {
    it('accepts a plugin with no locales', () => {
      const pluginClone = deepClone(nicePlugin);
      delete pluginClone.locales;
      const pluginJson = ConfigPlugin.parsePlugin(pluginClone, nicePluginName, nicePluginPath);
      assert.ok(pluginJson);
    });

    it('throws if locales is not an object', () => {
      const pluginClone = deepClone(nicePlugin);
      pluginClone.locales = [0, 1, 2, 3];
      assert.throws(() => ConfigPlugin.parsePlugin(pluginClone, nicePluginName, nicePluginPath),
        /^Error: lighthouse-plugin-nice-plugin locales json is not defined as an object\./);
    });

    it('throws if locales contains invalid locale', () => {
      const pluginClone = deepClone(nicePlugin);
      pluginClone.locales['pig-lat'] = {};
      assert.throws(() => ConfigPlugin.parsePlugin(pluginClone, nicePluginName, nicePluginPath),
        /^Error: lighthouse-plugin-nice-plugin contains unsupported locale: 'pig-lat'\./);
    });

    it('throws if locale value is not an object', () => {
      const pluginClone = deepClone(nicePlugin);
      pluginClone.locales['en'] = 42;
      assert.throws(() => ConfigPlugin.parsePlugin(pluginClone, nicePluginName, nicePluginPath),
        /^Error: lighthouse-plugin-nice-plugin locale: 'en' is not an object\./);
    });

    it('throws if locale message value is not an object', () => {
      const pluginClone = deepClone(nicePlugin);
      pluginClone.locales['en'] = {'audits.test.js | title': 'Title'};
      assert.throws(() => ConfigPlugin.parsePlugin(pluginClone, nicePluginName, nicePluginPath),
        /^Error: lighthouse-plugin-nice-plugin locale: 'en' contains invalid message: 'audits\/test.js | title'\./);
    });

    it('throws if locale contains message value with excess properties', () => {
      const pluginClone = deepClone(nicePlugin);
      pluginClone.locales['en']['audits/test.js | Title'].spiritAnimal = 'Mongoose';
      assert.throws(() => ConfigPlugin.parsePlugin(pluginClone, nicePluginName, nicePluginPath),
        /^Error: lighthouse-plugin-nice-plugin has unrecognized locale properties:.*spiritAnimal.*/);
    });

    it('correctly passes through the contained locales', () => {
      const pluginJson = ConfigPlugin.parsePlugin(nicePlugin, nicePluginName, nicePluginPath);
      const locales = pluginJson.pluginLocales;
      assert.deepStrictEqual(
        locales, {'en': {'nice-plugin/audits/test.js | Title': {message: 'Test Audit'}}});
    });
  });
});
