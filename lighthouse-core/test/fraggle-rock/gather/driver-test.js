...
for (const fnName of DELEGATED_FUNCTIONS) {
  describe(fnName, () => {
    ...
    it('should use connected session for default', async () => {
      ...
      if (!driver._defaultSession) throw new Error('Driver did not connect');

      /** @type {any} */
      const args = [1, {arg: 2}];
      const returnValue = {foo: 'bar'};
      driver._defaultSession[fnName] = fnAny().mockReturnValue(returnValue);
      // @ts-expect-error - typescript can't handle this union type.
      const actualResult = driver.defaultSession[fnName](...args);
      expect(driver._defaultSession[fnName]).toHaveBeenCalledWith(...args);
