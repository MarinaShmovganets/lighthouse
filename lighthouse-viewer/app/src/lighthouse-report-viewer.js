...
/* global ga, logger, GithubApi, DragAndDrop */
...
class LighthouseViewerReport extends LighthouseReport {
  ...
  constructor() {
    ...
    this.dragAndDropper = new DragAndDrop(this.onFileUpload);
  ...
  _addEventListeners() {
    ...
    const fileInput = document.querySelctor('#hidden-file-input');
    if (fileInput) {
      fileInput.addEventListener('change', e => this.onFileUpload(e.target.files[0]));
    }

    // A click on the placeholder will trigger the hidden file input.
    const placeholder = document.querySelector('.viewer-placeholder');
    if (fileInput && placeholder) {
      placeholder.firstElementChild.addEventListener('click', e => {
  ...
  loadFromDeepLink() {
    ...
    if (gistId) {
      this.github.getGistFileContentAsJson(gistId).then(reportJson => {
        this._isNewReport = false;
        this.replaceReportHTML(reportJson.content);
   ...
   */
  onFileUpload(file) {
    return LighthouseViewerReport.readFile(file).then(str => {
      let json;
      try {
        json = JSON.parse(str);
      } catch(e) {
